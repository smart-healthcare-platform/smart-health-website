...
export default function AddDoctorDialog({ isOpen, onClose, onSave, onAdded }: AddDoctorDialogProps) {
  ...
  /** ---------------------------------------------------
   *  CHỈNH SỬA THEO DTO MỚI (ĐÃ CLEAN)
   * --------------------------------------------------*/
  const prepareCertificatePayload = (cert: any, doctorId: string): CreateDoctorCertificateDto => {
    const base: CreateDoctorCertificateDto = {
      doctor_id: doctorId,
      type: cert.type,
      description: cert.description || "",
      certificate_file: cert.certificate_file || undefined,
      // Fields đặc thù
    ...
    if (cert.type === CertificateType.DEGREE) {
      base.academic_degree = cert.academic_degree || ""
      base.field = cert.field || ""
      base.graduation_year = cert.graduation_year ? Number(cert.graduation_year) : undefined
    }

    if (cert.type === CertificateType.LICENSE) {
      base.license_number = cert.license_number || ""
      base.issued_by = cert.issued_by || ""
      base.issued_date = cert.issued_date || undefined
      base.expiry_date = cert.expiry_date || undefined
    }
  ...
  const handleSave = async () => {
    try {
      ...
      if (!doctor) throw new Error("Doctor response null")
        ...
        <DialogContent className="w-[90vw] max-w-[1200px] h-[650px] p-6 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <DialogFooter className="mt-4 flex justify-between gap-2">
              <Button variant="outline" onClick={prevStep} disabled={currentStep === 1}>Quay lại</Button>

              {currentStep < 4 ? (
                <Button onClick={() => isStep1Valid && nextStep()} disabled={!isStep1Valid}>Tiếp theo</Button>
              ) : (
                <Button onClick={handleSave}>Lưu bác sĩ</Button>
              )}
            </DialogFooter>
          </div>
        </DialogContent>
