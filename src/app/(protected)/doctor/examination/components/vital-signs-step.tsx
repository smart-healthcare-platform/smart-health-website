...
import { Weight, Ruler, Thermometer, Droplets } from "lucide-react"
...
export function VitalSignsStep({ data, onUpdate, onNext, onPrevious }: VitalSignsStepProps) {
  ...
  const handleBlur = (field: keyof VitalSigns, value: string | number) => {
    const num = typeof value === "string" ? Number(value) : value
    let error = ""

    switch (field) {
      case "systolicPressure":
        if (!num || num < 90 || num > 200) error = "Tâm thu phải từ 90–200 mmHg"
        break
      case "diastolicPressure":
        if (!num || num < 60 || num > 120) error = "Tâm trương phải từ 60–120 mmHg"
        break
      case "temperature":
        if (!num || num < 35 || num > 42) error = "Nhiệt độ phải từ 35–42 °C"
        break
      case "oxygenSaturation":
        if (!num || num < 70 || num > 100) error = "SpO₂ phải từ 70–100%"
        break
      case "weight":
        if (!num || num <= 0 || num > 500) error = "Cân nặng phải từ 1–500 kg"
        break
      case "height":
        if (!num || num <= 30 || num > 250) error = "Chiều cao phải từ 30–250 cm"
        break
    }
  ...
  // --- Disable button nếu có lỗi hoặc thiếu dữ liệu ---
  const isFormValid =
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={(e) => handleChange("systolicPressure", e.target.value ? Number(e.target.value) : "")}
              onBlur={(e) => handleBlur("systolicPressure", e.target.value)}
            />
            {errors.systolicPressure && <p className="text-red-500 text-sm">{errors.systolicPressure}</p>}
          </div>
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={(e) => handleChange("diastolicPressure", e.target.value ? Number(e.target.value) : "")}
              onBlur={(e) => handleBlur("diastolicPressure", e.target.value)}
            />
            {errors.diastolicPressure && <p className="text-red-500 text-sm">{errors.diastolicPressure}</p>}
          </div>
        </div>
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            onBlur={(e) => handleBlur("oxygenSaturation", e.target.value)}
          />
          {errors.oxygenSaturation && <p className="text-red-500 text-sm">{errors.oxygenSaturation}</p>}
        </div>
        ...
        {/* BMI */}
        {formData.bmi && (
          <div className="col-span-2 p-3 rounded-lg bg-muted flex items-center justify-between">
            <div>
