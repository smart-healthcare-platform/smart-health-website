...
import { requestNotificationPermission, onMessageListener, isNotificationSupported } from '@/lib/firebase';
...
export function NotificationProvider({ children, userId }: NotificationProviderProps) {
  ...
  useEffect(() => {
    if (userId && !isListening) {
      const initNotifications = async () => {
        ...
        if (!isAuthenticated()) {
          console.log('[NotificationProvider] User not authenticated yet, waiting...');
          return;
        }
  ...
  return (
    <>
      ...
      {userId && permissionStatus === 'default' && isNotificationSupported() && (
